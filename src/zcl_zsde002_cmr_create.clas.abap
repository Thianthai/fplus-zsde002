CLASS zcl_zsde002_cmr_create DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  ...
  PUBLIC SECTION.
    ...
    " สร้าง Credit Memo Request (SD Document Category = K) ผ่าน I_CreditMemoRequestTP
...
CLASS zcl_zsde002_cmr_create IMPLEMENTATION.

  METHOD zif_zsde002_doc_create~create.

    DATA lt_header        TYPE TABLE FOR CREATE i_creditmemorequesttp.
    DATA lt_reference     TYPE TABLE FOR ACTION IMPORT i_creditmemorequesttp~createwithreference.
    DATA lt_partner       TYPE TABLE FOR CREATE i_creditmemorequesttp\_Partner.
    DATA lt_headerpricing TYPE TABLE FOR CREATE i_creditmemorequesttp\_PricingElement.
    DATA lt_headertext    TYPE TABLE FOR CREATE i_creditmemorequesttp\_Text.
    DATA lt_item          TYPE TABLE FOR CREATE i_creditmemorequesttp\_Item.
    DATA lt_itempricing   TYPE TABLE FOR CREATE i_creditmemorequestitemtp\_ItemPricingElement.
    DATA lt_itemtext      TYPE TABLE FOR CREATE i_creditmemorequestitemtp\_ItemText.

    DATA ls_partner       TYPE STRUCTURE FOR CREATE i_creditmemorequesttp\_Partner.
    DATA ls_headerpricing TYPE STRUCTURE FOR CREATE i_creditmemorequesttp\_PricingElement.
    DATA ls_headertext    TYPE STRUCTURE FOR CREATE i_creditmemorequesttp\_Text.
    DATA ls_item          TYPE STRUCTURE FOR CREATE i_creditmemorequesttp\_Item.
    DATA ls_itempricing   TYPE STRUCTURE FOR CREATE i_creditmemorequestitemtp\_ItemPricingElement.
    DATA ls_itemtext      TYPE STRUCTURE FOR CREATE i_creditmemorequestitemtp\_ItemText.
    ...
    ELSE.
      ...
      APPEND VALUE #( %cid                      = gc_cid_header
                      CreditMemoRequestType     = is_order-sales_order_type
                      ...
                      CreditMemoRequestDate     = to_internal_date( is_order-document_date )
    ...
    LOOP AT it_item ASSIGNING FIELD-SYMBOL(<lfs_item>).
      ...
      APPEND VALUE #( %cid                          = lv_item_cid
                      ...
                      CreditMemoRequestItemCategory = <lfs_item>-item_category
    ...
    MODIFY ENTITIES OF i_creditmemorequesttp PRIVILEGED
      ENTITY CreditMemoRequest
        ...
        CREATE FIELDS ( CreditMemoRequestType
                        SalesOrganization
                        DistributionChannel
    ...
    COMMIT ENTITIES BEGIN
      RESPONSE OF i_creditmemorequesttp
      FAILED   DATA(ls_commit_failed)
      REPORTED DATA(ls_commit_reported).

    IF ls_commit_failed-creditmemorequest IS INITIAL.
      LOOP AT ls_mapped-creditmemorequest ASSIGNING FIELD-SYMBOL(<lfs_key>).
        CONVERT KEY OF i_creditmemorequesttp FROM <lfs_key>-%pid TO <lfs_key>-%key.
      ENDLOOP.
    ENDIF.
    ...
    " SD Document Number -----------------------------------------------
    LOOP AT ls_mapped-creditmemorequest ASSIGNING <lfs_key>.
      rs_result-sales_order_number = <lfs_key>-CreditMemoRequest.
    ENDLOOP.
